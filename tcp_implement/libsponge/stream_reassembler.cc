 ...
 * @LastEditTime: 2024-08-12 17:34:02
 * @LastEditors: zy
 */
...
using namespace std;
// _output 是 ByteStream对象，用于存储重组后的有序字节流
...
void StreamReassembler::_buf_erase(const set<Segment>::iterator &iter) {
    _unassembled_bytes -= iter->length();  // 减少了未被重组的字节数
    _buf.erase(iter);
}
...
//处理接收到的片段，检查片段是否在可接受范围内，并处理部分重叠或完全覆盖的情况
